...
library(reginv)
ft = est_cutt(deer,deerSds,K=14250) # takes maybe a minute to run
ft
```

This returns a point estimate for extinction of `r format(ft$theta[2], scientific=FALSE)` BP, and by default, it also includes a 95% confidence interval for it, which in this case extends a couple of centuries either side of `r format(ft$theta[2], scientific=FALSE)` BP.
...
While this looks fairly symmetrical, the model we are fitting actually expects this distribution to be left-skewed:
...
ecostats::qqenvelope(qnorm(pcutt(deer,ft$theta[2],sd=deerSds,K=14250,df=ft$df)))
```
